...
class MessageAdpater(val context: Context, val list : List<MessageModel>)
    : RecyclerView.Adapter<MessageAdpater.MessageViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) : MessageViewHolder{
        return if(viewType == MSG_TYPE_RIGHT) {
            ...
            MessageViewHolder(LayoutInflater.from(context).inflate(R.layout.layout_seeder_message, parent, false))
        }
    }
            ...
            .child(list[position].senderId!!).addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if(snapshot.exists()) {
                        ...
                        Glide.with(context).load(data!!.image).placeholder(R.drawable.profilecl).into(holder.image)
